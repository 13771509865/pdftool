	...
	public void handleReturnValue(Object returnValue,
		...
		for (int i = 0; i < advices.size(); i++) {
			...
			if (ad.supports(returnType, null)) {
				returnValue = ad.beforeBodyWrite(
						returnValue,
						returnType,
						MediaType.APPLICATION_JSON_UTF8,
						null,
						new ServletServerHttpRequest(webRequest
								.getNativeRequest(HttpServletRequest.class)),
						new ServletServerHttpResponse(webRequest
		...
		for (Annotation a : annos) {
			if (a instanceof JSON) {
				JSON json = (JSON) a;
				jsonSerializer.filter(json.type(), json.include(),
						json.filter());
			}
		}
	...
	public Object postProcessAfterInitialization(Object bean, String beanName)
		...
		if (bean instanceof ResponseBodyAdvice) {
			...
			if (jsonHandler != null) {
				...
				((RequestMappingHandlerAdapter) bean)
						.setReturnValueHandlers(handlers); // change the
															// jsonhandler sort
			}
		}
		return bean;
