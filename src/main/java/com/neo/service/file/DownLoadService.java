	...
	public IResult<String> buildVToken(ViewTokenBO viewTokenBO){
		try {
			...
			if (!HttpUtils.isHttpSuccess(httpResult)) {
				return DefaultResult.failResult(EnumResultCode.E_FCS_CONVERT_FAIL.getInfo());
			}
			...
			SysLogUtils.error(EnumResultCode.E_FCS_VTOKEN_FAIL.getInfo(), e);
			return DefaultResult.failResult(EnumResultCode.E_FCS_VTOKEN_FAIL.getInfo());
		}
	}
