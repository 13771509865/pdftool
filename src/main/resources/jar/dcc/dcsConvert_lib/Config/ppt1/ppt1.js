...
play = function(index) {
	...
	if(datas[index].isLoad == "false"){
		...
		if (isPhone) {
        	$('.word-content').html('<img src="' + data.url + '" width="100%"/>');
		} else {
			$('.word-content').html('<embed src="' + data.url + '" width="100%" type="image/svg+xml"></embed>');
        }
...
function afterLoad(url,index){
	$.ajax({
	  ...
	  success:function(data){
	  	  if (isPhone) {
        	$('.word-content').html('<img src="' + url + '" width="100%"/>');
		  } else {
			$('.word-content').html('<embed src="' + url + '" width="100%" type="image/svg+xml"></embed>');
		  }
          datas[index].isLoad = "true";
          pageChangeScroll(index);
	  },
	  error:function(data){
		  $('.word-content').html('<img src="' + basePath + '/loading.gif" width="100%"/>');
		  console.clear();
	  }	  
	});
}
function pageChangeScroll(index){
	...
	if(currentThumbnail.position().top + currentThumbnail.height() - scrollHeight < 0 || currentThumbnail.position().top - screenHeight - scrollHeight > 0){
		...
		sidebar.animate({scrollTop:(pos)}, 0);
	}
}
