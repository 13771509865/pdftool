...
function initialCanvasParameter(e) {
	...
	$.each(canvas, function (index, ele) {
		if (canvas[index].classList.contains('active') || canvas2[index].classList.contains('active')) {
			context[index].strokeStyle = font_color;
			context[index].lineWidth = line_width;
			context2[index].strokeStyle = font_color;
			context2[index].lineWidth = line_width;
		}
	})
	// context[0].strokeStyle = font_color;
	// context[0].lineWidth = line_width;
	// context2[0].strokeStyle = font_color;
	// context2[0].lineWidth = line_width;
}
...
function openSignature() {
	...
	if ($(".word-page:visible").find('canvas').length == 0) {
		$(".word-page:visible").append("<canvas id='draw_canvas' class='draw_canvas'>您的浏览器不支持画布</canvas><canvas id='cache_canvas' class='cache_canvas'>您的浏览器不支持画布</canvas>");
	}
	// canvas = document.getElementById("draw_canvas");
	// canvas2 = document.getElementById("cache_canvas");
	// context = canvas.getContext('2d');
	// context2 = canvas2.getContext('2d');
	canvas = document.getElementsByClassName('draw_canvas');
	canvas2 = document.getElementsByClassName('cache_canvas');
	$.each(canvas, function (index, ele) {
...
function saveFile() {
	...
	for (var i = 0; i < canvas.length; i++) {
		...
		reDrawCanvas(saveCanvas, false, i);
		saveCanvasArr.push(saveCanvas);
	}
	// var saveCanvas = document.createElement("canvas");
	// saveCanvas.width = fileOriginalWidth;
	// saveCanvas.height = fileOriginalHeight;
	...
	$("#loading").show();
	saveSuccessCallBack(saveCanvasArr);
}

function reDrawCanvas(saveCanvas, iszoomCanvas, index) {
	if (step == 0) {
	...
	for (var i = 0; i < thisDrawStatus.lineArr.length; i++) {
		...
		var thisplace = thisLine.place;
		if (thisplace == index) {
			saveCanvas.flag = true;
			saveCanvasContext.lineWidth = thisLine.line_width;
			saveCanvasContext.strokeStyle = thisLine.line_color;
			drawType = 0;
	...
	for (var i = 0; i < thisDrawStatus.straightLineArr.length; i++) {
		...
		var thisplace = thisStrightLine.place;
		if (thisplace == index) {
			saveCanvas.flag = true;
	...
	for (var i = 0; i < thisDrawStatus.circleArr.length; i++) {
		...
		var thisplace = thisCircleLine.place;
		if (thisplace == index) {
			saveCanvas.flag = true;
	...
	for (var i = 0; i < thisDrawStatus.rectArr.length; i++) {
		...
		var thisplace = thisRectLine.place;
		if (thisplace == index) {
			saveCanvas.flag = true;
	...
	for (var i = 0; i < thisDrawStatus.arrowArr.length; i++) {
		...
		var thisplace = thisArrow.place;
		if (thisplace == index) {
			saveCanvas.flag = true;
			saveCanvasContext.fillStyle = thisDrawStatus.arrowArr[i].line_color;
			drawType = 4;
			arrowCoord({
				x: thisArrow.origin.x * saveCanvas.width,
				y: thisArrow.origin.y * saveCanvas.height
			}, {
					x: thisArrow.end.x * saveCanvas.width,
					y: thisArrow.end.y * saveCanvas.height
				},
				CONST.edgeLen);
			sideCoord();
	...
	if (!iszoomCanvas) {
		...
		for (var i = 0; i < thisDrawStatus.textArr.length; i++) {
			...
			var thisplace = thisDrawStatus.textArr[i].place;
			if (thisplace == index) {
				saveCanvas.flag = true;
...
function saveSuccessCallBack(saveCanvasArr) {

	//window.open(saveCanvas.toDataURL("image/png"), "_blank")
	var dataArr = [];
	var refreshArr = [];
	var pageNum = JSON.stringify(saveCanvasArr.length);
	$.each(saveCanvasArr, function (index, ele) {
		if (ele.flag) {
			var canvasNum = index;
			var base64 = saveCanvasArr[index].toDataURL('img/png');
			dataArr.push(JSON.stringify({ 'canvasNum': canvasNum, 'base64': base64 }));
			refreshArr.push(canvasNum);
		} else {
			var canvasNum = index;
			var base64 = null;
			dataArr.push(JSON.stringify({ 'canvasNum': canvasNum, 'base64': base64 }))
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data.result == 0) {
				...
				var refreshUrl = thisUrl.substring(thisUrl.lastIndexOf("/")+1);
				fileUrl = data.data;
				$("#download").attr("href", data.data);
				$.each(refreshArr,function(index,ele){
					var thisSrc = basePath + "/" + (ele + 1) + "." + imgType + "?ver=" + Math.random()
//				$(pageImage).bind("load", function () {
					if ("svg" == imgType) {
						$(".word-content").eq(ele).find("embed").remove();
						$(".word-content").eq(ele).append('<embed src="' + thisSrc + '" width="100%" height="100%" type="image/svg+xml"></embed>')
					} else if ("png" == imgType) {
						$(".word-content").eq(ele).find("img").remove();
                        $(".word-content").eq(ele).append('<img src="' + thisSrc + '" width="100%" height="100%" />')
					}
//				})
				 });
				closeSignature();
...
function pmToEc(msg) {
	...
	if (window.location.search == "") return;
	var url = "http://" + window.location.search.substring(5);
...
function saveTextArea() {
	...
	$.each(canvas, function (index, ele) {
		if ($('.word-page').eq(index).find('.op-frame').length > 0) {
			place = index;
		}
	})
	if (!$(".op-frame.active").attr("data-save-step")) {
		...
		drawStatus.textArr.push({
			"width": $(".op-frame.active").width() / canvas[0].width,
			"height": $(".op-frame.active").height() / canvas[0].height,
			"top": textAreaData.top / canvas[0].height,
			"left": ($(".op-frame.active").offset().left - canvas_offset.x) / canvas[0].width,
	...
	$(".op-inter,.op-ctl-p").unbind();
	$.each(canvas, function (index, ele) {
		if ($('.word-page').eq(index).find('.op-frame').hasClass('fixed')) {
			opt = index;
		}
	})
	canvasDataArray.push({
		"type": drawType,
		"data": $(".draw_canvas")[opt].toDataURL("image/png"),
		"step": step,
		"opt": opt
	});
}
...
function recoverCanvas() {
	...
	if (canvasDataArray[step - 1].type != 5) {
		...
		img.src = canvasDataArray[step - 1].data;
		$(img).bind("load", function () {
			var fg = canvasDataArray[step - 1].opt;
			context[fg].drawImage(this, 0, 0, this.width, this.height);
		});
	} else if (canvasDataArray[step - 1].type == 5) {
		var fg = canvasDataArray[step - 1].opt;
		$("#textAreaTemplate .op-frame").css({
			"width": drawStatus.textArr[drawStatus.textArr.length - 1].width * canvas[0].width + "px",
			"height": drawStatus.textArr[drawStatus.textArr.length - 1].height * canvas[0].height + "px",
			"top": drawStatus.textArr[drawStatus.textArr.length - 1].top * canvas[0].height + "px",
			"left": drawStatus.textArr[drawStatus.textArr.length - 1].left * canvas[0].width + "px",
		...
		$(".word-page:visible").eq(fg).append($("#textAreaTemplate").html());
	}
...
function widthdrawCanvas() {
	...
	if (canvasDataArray[step - 1].type != 5) {
		var flag = canvasDataArray[step - 1].opt;
		clear_canvas(flag);
		step = step - 1;
		if (step >= 1) {
			var img = new Image();
...
function clear_canvas(index) {
	context[index].clearRect(0, 0, canvas_size.x, canvas_size.y);
}
...
function startBindCanvas() {
	$("canvas").bind('mousedown touchstart', function (event) {
		event.preventDefault();
		if (drawType != 5) {
			$(this).addClass('active').siblings('.draw_canvas').addClass('active');
		}
		var havePopUP = $(".fontsize-select").is(':visible') || $(".color-select").is(':visible') || $(".hideToolbar").is(':visible');
		...
		if (drawType == 0) {
			lines.push({
				"x": origin.x / canvas[0].width,
				"y": origin.y / canvas[0].height,
			})
		}
		if (drawType == 5) {
	...
	$("canvas").bind("mousemove touchmove", function (event) {
		...
		if (drawType == 0) {
			...
			$.each(canvas, function (index, ele) {
				if (canvas[index].classList.contains('active')) {
					draw(context[index]);
				}
			})
			lines.push({
				"x": end.x / canvas[0].width,
				"y": end.y / canvas[0].height,
			})
			origin.x = end.x;
			origin.y = end.y;
			...
			$.each(canvas2, function (index, ele) {
				if (canvas2[index].classList.contains('active')) {
					context2[index].clearRect(0, 0, canvas_size.x, canvas_size.y);
					draw(context2[index]);
				}
			})
		} else if (drawType == 4) {
			...
			$.each(canvas2, function (index, ele) {
				if (canvas2[index].classList.contains('active')) {
					context2[index].clearRect(0, 0, canvas_size.x, canvas_size.y);
					context2[index].fillStyle = font_color;
					draw(context2[index]);
				}
			})
		}
	...
	$("canvas").bind('mouseup touchend', function (event) {
		...
		if (drawType == 0 && drawable == true) {
			...
			$.each(canvas, function (index, ele) {
				if (canvas[index].classList.contains('active')) {
					canvas[index].className = 'draw_canvas';
					canvas2[index].className = 'cache_canvas';
					opt = index;
					place = index;
				}
			})
		...
		if ((drawType == 1 || drawType == 2 || drawType == 3) && drawable == true) {
			...
			thisDrawStatus.push({
				"origin_x": origin.x / canvas[0].width,
				"origin_y": origin.y / canvas[0].height,
				"end_x": end.x / canvas[0].width,
				"end_y": end.y / canvas[0].height,
				"line_width": line_width,
				"line_color": font_color,
				"place": place
			});
		...
		if (drawType == 4 && drawable == true) {
			...
			$.each(canvas2, function (index, ele) {
				if (canvas2[index].classList.contains('active')) {
					context2[index].clearRect(0, 0, canvas_size.x, canvas_size.y);
					context[index].fillStyle = font_color;
					draw(context[index]);
					canvas[index].className = 'draw_canvas';
					canvas2[index].className = 'cache_canvas';
					opt = index;
