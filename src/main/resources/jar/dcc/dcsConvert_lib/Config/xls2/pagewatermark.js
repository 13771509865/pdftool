...
function watermark(settings) {
	...
	// 获取页面最大宽度
	var page_width = documentTotalWidth + documentColumn * 10;
	// 获取页面最大长度
	var page_height = documentTotalHeight + documentRow * 10 + 1000;
	var x;
	var y;
	// 预处理
	var fontSize = defaultSettings.watermark_fontsize;
	var watermarkWidth = fontSize.substring(0, fontSize.length - 2)
			* defaultSettings.watermark_txt.length;
	...
	for (var i = 0; i < defaultSettings.watermark_rows; i++) {
		y = defaultSettings.watermark_y
				+ (defaultSettings.watermark_y_space + defaultSettings.watermark_y_extra)
				* i;
		for (var j = 0; j < defaultSettings.watermark_cols; j++) {
			x = defaultSettings.watermark_x
					+ (defaultSettings.watermark_x_space + defaultSettings.watermark_x_extra)
					* j;
			...
			// mask_div.style.filter =
			// "progid:DXImageTransform.Microsoft.Matrix(sizingMethod='auto
			// expand', M11=0.7071,M12=0.7071, M21=-0.7071,
			// M22=0.7071)progid:DXImageTransform.Microsoft.Alpha(opacity="+defaultSettings.watermark_alpha
			// * 100+")";
			...
			// mask_div.style.lineHeight = defaultSettings.watermark_height +
			// 'px';
