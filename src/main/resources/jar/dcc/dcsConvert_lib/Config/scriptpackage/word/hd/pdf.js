...
$(window).resize(function (e) {
	var wid = $('.container-fluid').width() - 76
	$('.title-box').width(wid)
	if (!isSignatureModel) {
		renderPage()
	} else {
	...
	$('.attribute').css('margin-top', (hei - 229) / 2)
	$('.docMark').height(hei)
})
...
function bindActiveIndex() {
	...
	$(".word-page").each(function (index, e) {
		if (index > 0) {
			if ($(this).position().top - 5 - scrollHeight < (screenHeight/2)) {
				activeIndex = index
			}
		}
	});
...
$(function () {
	...
	// $('#fileType').text(attribution.fileType)
	// $('#docSize').text(attribution.docSize)
	// $('#modifyTime').text(attribution.modifyTime)
	// $('#createTime').text(attribution.modifyTime)
	$(".attr_detail").each(function (index, e) {
		$(this).html(attribution[index])
	})

	var wid = $('.container-fluid').width()
	$('.title-box').width(wid - 76).find('.brand').css('display', 'inline-block')
	var hei = $(window).height()
	$('.attribute').css('margin-top', (hei - 229) / 2)
	$('.docMark').height(hei)
	if (authority.isShowList) {
		$('#toggleSidebar').css('visibility', 'visible')
	}
	if (authority.isPrint) {
		$(".btnPrint").show()
	}
	if (authority.showFooter && !(bIsIpad || bIsIphoneOs || bIsMidp || bIsUc7 || bIsUc || bIsAndroid || bIsCE || bIsWM)) {
		$('#footer').show()
		clearInterval(timer)
	} else {
		setTimeout(function () {
			$('#zoom').css('bottom', '35px')
		}, 1)
	}
	if (authority.isDownload) {
		$('.download').show()
	}
});
